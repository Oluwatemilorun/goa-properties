	...
	<v-container white fluid fill-height>
		<v-layout align-center justify-center>
			<v-flex xs10 sm8 md4 xl3>
				<v-container>
					<h3 class="font-weight-light display-1 text-xs-center">Login</h3>
				</v-container>
				<v-container >
				...
				<v-container >
					<p class="text-xs-center">
						...
						<v-btn
							...
							ripple flat
							@click.native="$router.replace('/signup')"
						>
							Create account
						</v-btn>
...
export default {
	...
	methods: {
		processLogin () {
			...
			firebase.auth().signInWithEmailAndPassword(`${this.email}@estatelinks.net`, this.password)
				.then(u => {
				...
				.catch(err => {
					...
					this.snackbar = true;
					this.snackbarText = 'An error occured while logging';
				});
		}
	}
}
